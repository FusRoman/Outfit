...
fn test_load_traj_from_parquet() {
    ...
    let traj_set = TrajectorySet::new_from_parquet(&path_file, ztf_observer, None);
...
fn test_large_parquet() {
    ...
    let mut traj_set = TrajectorySet::new_from_parquet(&path_file, ztf_observer, None);
    ...
    traj_set.add_from_parquet(path_file, rubin_observer, None);

    assert_eq!(traj_set.len(), 2082);
    assert_eq!(traj_set.get(&ObjectNumber::Int(1)).unwrap().len(), 9);
